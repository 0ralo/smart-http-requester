...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if client is None:
            return JSONResponse(status_code=status.HTTP_423_LOCKED, content={"detail": "Cannot get ip address"})
        ip = client.host
        allowed, total, reset_time = await check_rate_limit(f"rate_limit:{ip}", settings.rate_limit, 60)
        if allowed:
