...
async def create_user(
    session: AsyncSession, data: UserRegisterBody
) -> UserRegisterResponse:
    ...
    try:
        ...
        logger.info(
            "User created successfully: username=%s user_id=%s",
            data.username,
            user.user_id,
        )
        return user
        ...
        if unique_violation_types and isinstance(e.orig, unique_violation_types):
            logger.warning(
                "User creation failed: username already exists (%s)", data.username
            )
            raise UserAlreadyExists
        logger.exception(
            "Unexpected database error while creating user %s", data.username
        )
        raise UnknownException


async def get_token(session: AsyncSession, data: UserLoginBody) -> AccessTokenResponse:
    repo = AuthRepository(session)
    ...
    if id_with_hash is None:
        logger.warning(
            "Token generation failed: username not found (%s)", data.username
        )
        raise UserDoesNotExists
    password_correct = check_password(data.password_hash, id_with_hash.password_hash)
    if not password_correct:
        logger.warning(
            "Token generation failed: invalid password for username=%s", data.username
        )
        raise PasswordIsIncorrect
    token = await repo.get_access_token_by_user_id(id_with_hash.user_id)
    logger.info(
        "Token generated successfully for username=%s user_id=%s",
        data.username,
        id_with_hash.user_id,
    )
...
async def get_token_for_docs(
    ...
    if not password_correct:
        logger.warning(
            "Docs token generation failed: invalid password for username=%s", login
        )
        raise PasswordIsIncorrect
    token = await repo.get_access_token_by_user_id(id_with_hash.user_id)
    logger.info(
        "Docs token generated successfully for username=%s user_id=%s",
        login,
        id_with_hash.user_id,
    )
    return AccessTokenResponse(access_token=str(token))
...
async def delete_token(session: AsyncSession, user_id: int) -> int:
    repo = AuthRepository(session)
...
async def refresh_token(session: AsyncSession, user_id: int) -> AccessTokenResponse:
    repo = AuthRepository(session)
...
async def get_user_info(session: AsyncSession, user_id: int) -> UserMe:
    repo = AuthRepository(session)
